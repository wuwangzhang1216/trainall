...
公式：$\mathcal{L}_\text{PT}=-\sum_t \log P_\theta(x_t \mid x_{\lt t})$
...
公式：$\mathcal{L}_\text{SFT}=-\frac{1}{|\mathcal{R}|}\sum_{t\in\mathcal{R}}\log p_\theta(x_t\mid x_{\lt t})$
...
### REINFORCE baseline
**REINFORCE 基线**  
朴素策略梯度 $\nabla\log\pi\cdot r$ 方差大（即使最差的答案只要 $r\gt 0$ 也会被推高）。**减去一个基线** $b$ 不改变梯度期望却大幅降方差。PPO 用 value 网络学 $b$；RLOO 用「其他样本的均值」当 $b$；GRPO 用「组内均值」当 $b$。
公式：$\nabla J=\mathbb{E}[(r-b)\nabla\log\pi]$
...
### dark knowledge
**暗知识**  
Hinton 等 (2015) 的说法：teacher 软标签里「非最大概率」的部分编码了**类间相似度结构**（把猫错认成狗比错认成汽车合理）。硬 one-hot 标签丢掉了这些信息，软标签把它传给 student，所以蒸馏比纯硬标签信息量大。需用温度 $T\gt 1$ 软化分布才能让这些小概率差异显现。
...
公式：$d\leftarrow\min(1,d+\text{step})$ 若 $\bar p\gt $ target_high；$\max(0,d-\text{step})$ 若 $\bar p\lt $ target_low
...
**下一个 token 目标**  
预训练/CPT/SFT 共享的核心目标：给定左侧上下文 $x_{\lt t}$ 预测第 $t$ 个 token，最小化负对数似然（交叉熵）。SFT 在它之上加 prompt 掩码（只在 response 上算）。模型自回归地把序列联合概率分解为 $\prod_t p_\theta(x_t\mid x_{\lt t})$。
公式：$\mathcal{L}=-\frac1N\sum_t\log p_\theta(x_t\mid x_{\lt t})$
...
公式：$\text{PPL}=\exp\big(\frac1N\sum_t-\log p_\theta(x_t\mid x_{\lt t})\big)$
...
**对数概率**  
模型分配给某 token/序列的概率取对数。序列对数概率 $\log\pi(y)=\sum_t\log\pi(y_t\mid y_{\lt t},x)$ 是偏好优化的基石（DPO 用它的差、SimPO 用其长度归一化版）。`trainall` 由 `sequence_logps(..., average=False/True)` 计算。
...
### sequence log-prob
**序列对数概率**  
一整条回答的对数概率 $\log\pi(y)=\sum_t\log\pi(y_t\mid y_{\lt t},x)$（`average=False`），或其长度归一化版 $\overline{\log\pi}(y)$（`average=True`，除以 token 数）。CPO 用未归一化版，SimPO/IPO/ORPO 用归一化版以消 length bias。
...
### temperature
**温度**  
采样/蒸馏里缩放 logits 的系数 $T$。采样时 $T\gt 1$ 更随机、$T\lt 1$ 更确定（贪心趋近 0）。蒸馏里 $T\gt 1$ 软化 teacher/student 分布让暗知识显现，且 KD 损失乘 $T^2$ 还原 $1/T^2$ 的梯度缩放；推理时 $T=1$。别把蒸馏的 $T$ 和推理采样的温度混淆。
...
### temperature scaling
**温度缩放**  
把 logits 除以 $T$ 再 softmax。蒸馏用它软化分布（$T\gt 1$ 放大小概率间的相对差异）；采样用它调随机性。`RolloutConfig.temperature` 控制 RL 采样温度。
