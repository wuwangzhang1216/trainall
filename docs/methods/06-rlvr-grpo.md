...
policy gradient 的方差主要来自奖励的绝对尺度。REINFORCE 的梯度是 $\nabla \log \pi \cdot r$，如果所有答案奖励都在 0.8 附近，那即使最差的答案也会被推高——因为它的 $r\gt 0$。**减去一个基线** $b$ 不改变梯度的期望（无偏），却能大幅降方差。PPO 用价值网络学这个 $b$；GRPO 直接用**组内均值** $\bar r_g$ 当 $b$，再除以组内标准差做归一化（z-score）。
