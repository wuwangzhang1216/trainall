...
其中 $\theta$ 是模型参数，$P_\theta(x_t \mid x_{<t})$ 是模型对位置 $t$ 真实 token 的预测概率，$\mathcal{M}$ 是参与计损的位置集合（标签为 `-100` 的位置被忽略，CPT 通常**所有 token 都计损**），$|\mathcal{M}|$ 是有效 token 数。这正是 [预训练](01-pretraining.md) 的损失。
