...
- $x_{<t}$：它前面的全部 token（含 prompt），作为条件。
- $\mathcal{R}$：response token 的下标集合；$|\mathcal{R}|$ 是其大小，用于按有效 token 数归一化。
- $p_\theta(x_t \mid x_{<t})$：模型在位置 $t$ 对 gold token 的预测概率，由 `log_softmax(logits)` 后 gather 得到。
...
其中 $V$ 是词表大小。$\varepsilon=0$ 退化为纯 NLL。平滑项把一点概率质量分给所有 token，缓解过自信、提升泛化与校准。`trainall` 里上报的 `ppl` 始终基于未平滑的 NLL：$\text{ppl} = \exp\big(\tfrac{1}{|\mathcal{R}|}\sum_{t\in\mathcal{R}} -\log p_\theta(x_t\mid x_{<t})\big)$。
