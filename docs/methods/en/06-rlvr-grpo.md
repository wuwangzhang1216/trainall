...
The variance of the policy gradient comes mainly from the absolute scale of the reward. The REINFORCE gradient is $\nabla \log \pi \cdot r$; if all answers have rewards around 0.8, then even the worst answer gets pushed up — because its $r\gt 0$. **Subtracting a baseline** $b$ does not change the expectation of the gradient (unbiased), yet greatly reduces variance. PPO learns this $b$ with a value network; GRPO directly uses the **within-group mean** $\bar r_g$ as $b$, then divides by the within-group standard deviation to normalize (z-score).
